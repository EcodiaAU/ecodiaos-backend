...
def ensure_keychain_password():
    ...
    If not in env, try VPS kv_store via SSH. Last resort: skip and let codesign
    fail with a clear error (better than the cryptic Security framework one).
    """
    ...
    try:
        ...
        if pw:
            ...
            print(
                f"[keychain] auto-loaded password from VPS kv_store ({len(pw)} chars)"
            )
...
def resolve_asv_id(spec):
    ...
    if not candidates:
        sys.exit(
            f"no submittable App Store version found at versionString={spec['marketing_version']}. "
...
def bump_build(spec):
    ...
    new = m + 1
    txt2 = re.sub(
        rf"CURRENT_PROJECT_VERSION = {m};", f"CURRENT_PROJECT_VERSION = {new};", txt
    )
...
def step_pull(spec):
    ...
    sh(
        f"git fetch origin main && git checkout main && git pull --ff-only {url} main",
        cwd=spec["build_dir"],
    )
...
def step_archive(spec, marketing_version, build_num):
    ...
    sh(
        f"""xcodebuild {workspace_flag} -scheme {spec["xcode_scheme"]} -configuration Release \\
...
def step_poll_build(spec, build_num, timeout=900):
    ...
    while time.time() < deadline:
        rc, j = api(
            spec,
            f"/v1/builds?filter[app]={spec['asc_app_id']}"
...
def step_submit_for_review(spec, asv_id):
    ...
    rc, j = api(
        spec,
        f"/v1/reviewSubmissions?filter[app]={spec['asc_app_id']}"
    ...
    if existing:
        sid = existing[0]["id"]
        print(
            f"reusing reviewSubmission {sid} state={existing[0]['attributes']['state']}"
        )
    else:
        rc, j = api(
            spec,
            ...
            body={
                "data": {
                    ...
                    "relationships": {
                        "app": {"data": {"type": "apps", "id": spec["asc_app_id"]}}
                    },
                }
            },
        )
        ...
        body={
            "data": {
                ...
                "relationships": {
                    ...
                    "appStoreVersion": {
                        "data": {"type": "appStoreVersions", "id": asv_id}
                    },
                },
            }
        },
    ...
    print(
        f"SUBMITTED. reviewSubmission {sid} now state={j.get('data', {}).get('attributes', {}).get('state', '?')}"
    )
    return sid
...
def main():
    ...
    if args.skip_bump:
        ...
        nums = [
            int(x)
            for x in re.findall(r"CURRENT_PROJECT_VERSION = (\d+);", open(proj).read())
        ]
    ...
    print(
        f"final submission state: {j['data']['attributes']['state']} submittedDate={j['data']['attributes']['submittedDate']}"
    )
    rc, j = api(spec, f"/v1/appStoreVersions/{asv_id}")
    print(
        f"App Store version state: {j['data']['attributes']['appStoreState']} releaseType: {j['data']['attributes']['releaseType']}"
    )
    print("DONE.")
