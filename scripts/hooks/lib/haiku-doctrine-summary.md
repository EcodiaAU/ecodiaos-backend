...
8. **macincloud-substrate-selection-ssh-vs-rdp** (7 May 2026, supersedes 5 May absolute SSH ban) - Select access substrate by what the work needs. SSH for headless work (git, scp, package installs, headless `xcodebuild`, log tails, DB migrations, `launchctl` ops, scripted tests, file CRUD, `pkill`). RDP from Corazon for GUI-bound work (Xcode IDE, ASC upload UI, `screencapture`, `cliclick` flows, AppleScript GUI calls, Messages.app interactive, Android Studio IDE - anything needing the GUI Aqua context). Tate paid the +AU$9/mo "Enable Remote Build Port (SSH)" add-on at ~11:28 AEST 7 May 2026. WARN-shape: brief plans GUI-bound work (`screencapture`, `cliclick`, AppleScript GUI, Messages.app interactive, Xcode IDE) over an SSH connection, OR conversely refuses SSH for headless `xcodebuild` / git / scp work and routes through RDP for no GUI reason.
