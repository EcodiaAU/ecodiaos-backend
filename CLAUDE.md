...
**THREE-account provider chain (updated 12 May 2026):** we have three Claude Max accounts wired. The fork provider router scores all three simultaneously and picks the best-scoring one. Provider chain (fallback of last resort only): `claude_max (tate@) → claude_max_2 (code@) → claude_max_3 → deepseek`. Selection is score-based, not fixed-sequential. Each account has two independent caps (weekly + 5h session). All three accounts exhausted concurrently = `account_chain_exhausted`, NOT a system outage. The conductor still processing proves at least one account has capacity. Full model: `~/ecodiaos/patterns/multi-account-credit-state-model.md`.
