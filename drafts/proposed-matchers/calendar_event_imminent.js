...
/**
 * Proposed perceptionDispatcher matcher: calendar_event_imminent
 *
 * fork_moslihvx_015515 — listener gap analysis 2026-05-05.
 *
 * Fires on heartbeat-class events. Queries calendar_events for events in
 * next 30min. Surfaces attendees + linked CRM context so Tate doesn't walk
 * into a meeting without prep. Critical for client calls.
 *
  ...
  async dispatch(event, ctx) {
    ...
    try {
      ...
      for (const evt of upcoming) {
        ...
        // Try to correlate attendee email → client
