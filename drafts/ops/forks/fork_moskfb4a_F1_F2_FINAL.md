...
| 3 | crm | `kind=crm_test`, data=`{client_id:"ord-craige"}` | YES (test() matched; dispatch ran but no derived event because crm_activities had 0 rows for that client_id) | confirmed via matcher logic |
