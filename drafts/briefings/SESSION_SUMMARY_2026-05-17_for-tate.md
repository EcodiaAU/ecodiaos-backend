...
1. **`gui.sequence` batch primitive** on the live laptop-agent (`D:/.code/eos-laptop-agent/tools/gui.js`, mirrored at `D:/.code/eos-laptop-agent/tools/gui.js`). Takes an array of `input.*` / `mouse.*` / `screenshot.*` / `wait` actions, runs them server-side, returns only the final screenshot. ~110 LOC. Auto-loaded by agent at boot, callable as `gui.sequence` via POST `/api/tool`.
...
**Restart procedure (for future tools/*.js edits) is documented in the auto-memory reference.** The PM2 metadata is half-broken on this machine; the clean recipe is `Stop-Process -Force <pid-on-7456>; pm2 delete; pm2 start ecosystem.config.js; pm2 save`. The aborted gauge prototype (`D:/.code/eos-laptop-agent/gui-worker/`) was scrapped earlier per your "no API" pivot.
...
- `D:/.code/eos-laptop-agent/tools/` mirror is missing 6+ files vs live - separate cleanup pass.
...
- `D:/.code/eos-laptop-agent/tools/gui.js` (mirror)
...
**Deleted:**
- `D:/.code/eos-laptop-agent/gui-worker/` (the aborted API-loop prototype, scrapped per your "no API" pivot)
