...
| **CMH3-haptics-package-installed-zero-imports** | CMH-3, A3 | P1 | `package.json:21` declares `@capacitor/haptics`; `capacitor.config.ts:18` registers `Haptics: {}`; **zero `.tsx`/`.ts` files import `@capacitor/haptics`** | Plugin is dead weight on the bundle. No haptic on RSVP, on tier purchase, on photo upload success, on member approve/reject | Wire `lib/haptics.ts` thin wrapper (`light()`, `medium()`, `success()`, `error()`) that no-ops on web. Call `light()` on every primary tap, `success()` on RSVP/profile-save/admin approve. Less is more — NOT on every scroll | M | **closed: 4130917** (F4, fork_morysylw_aa9964, 5 May 2026; 12 importing files, 39 light + 17 success + 18 error call sites; vitest unit tests pass; real-device verification deferred to next TestFlight/Play preview build per ~/ecodiaos/patterns/never-use-ssh-on-macincloud-rdp-only.md). EventDetail.tsx + EventsAdmin.tsx haptics edits ride along with sibling F3's pending commit (working-tree-only at ship time). |
...
| **CMH7-no-event-cover-upload** | D2, CMH-7 | P2 | `src/pages/admin/EventsAdmin.tsx` (no upload field); `tenant_events.cover_url` column exists; `Events.tsx:36-49` falls back to gradient | Officers cannot upload event cover images from the admin UI; placeholder gradient is the permanent state | Add file-upload field to event form; mirror `BrandingAdmin.handleLogoUpload` pattern; store under `tenant-assets/<tenant_id>/events/<event_id>.<ext>` | M | **closed: 14332a2** (F3-redo, fork_mos0riap_f5910d, 5 May 2026; Supabase Storage upload to `tenant-assets/<tenant>/events/<slot>-<ts>.<ext>` mirroring BrandingAdmin pattern; thumbnail preview + Replace/Remove; cover_url persists via useUpsertEvent; size + MIME validation [PNG/JPG/WebP/AVIF, 8MB cap]) |
...
| **CMH10-rsvp-presence-not-shown-on-event** | CMH-10 supporting | P2 | `src/pages/EventDetail.tsx` shows count only | Without showing who's attending, users can't anticipate connections | Add "Going" avatar strip on EventDetail (subject to a per-tenant privacy toggle in admin config) | M | **closed: 14332a2** (F3-redo, fork_mos0riap_f5910d, 5 May 2026; new useEventGoingMembers hook joins tenant_members for name/photo; stacked-avatar GoingStrip on EventDetail; click opens AttendeesModal listing all; privacy-gated by `tenant.privacy_settings.show_event_attendees_publicly ?? true` — F5 owns the column DEFAULT) |
| **CMH11-tiktok-not-in-admin-config** | CMH-11, D2 | P2 | `src/pages/admin/BrandingAdmin.tsx:386-393` lists facebook/instagram/linkedin/twitter/youtube + website only | Rubric & Tate verbatim mention socials; TikTok is widespread for chambers (especially youth-focused like SCYCC); also missing Threads/Bluesky which are now common | Add `social_tiktok` (and optionally `social_threads`, `social_bluesky`) columns to `tenants`; add fields to `BrandingAdmin`; add icons to `Home.tsx` social section + `Resources.tsx`. Update `BrandingForm` type | S | **closed: e47b6a7** (F5, fork_mos0swkk_564f27, 5 May 2026; +social_tiktok/social_threads/social_bluesky on tenants; BrandingAdmin Social section extended; Home.tsx + Resources.tsx render Music2/AtSign/Cloud lucide icons as TikTok/Threads/Bluesky surrogates until brand-icon set ships) |
| **CMH11-no-cover-image-in-admin-config** | CMH-11, D2 | P2 | `src/pages/admin/BrandingAdmin.tsx` has logo upload but NO `hero_image_url` upload widget despite `BrandingForm:31` declaring `hero_image_url` | Tenants set hero via SQL only — UI only changes hero text, not the hero image | Add cover image uploader mirroring logo path (target `tenant-assets/<tenant_id>/hero.<ext>`) | S | **closed: e47b6a7** (F5, fork_mos0swkk_564f27, 5 May 2026; handleHeroUpload mirrors handleLogoUpload, target `tenant-assets/<tenant_id>/hero.<ext>`, cache-busted public URL; Home.tsx renders hero image as `<img>` with dark gradient overlay when non-null, falls back to flat-primary hero otherwise) |
| **CMH11-no-notification-prefs-in-admin-config** | CMH-11, D2 | P2 | no UI; `tenant_events` has no notify-flag; `event-reminders` Edge Function exists but no admin toggles | Officers can't configure when push/email reminders fire (e.g. "send 24h before"), or whether members can mute by category | Add `tenant_notification_settings` table (event_reminder_offset_hours, allow_member_mute, send_via_push, send_via_email); render in admin (new sub-page `Admin → Notifications`) | M | **closed: e47b6a7** (F5, fork_mos0swkk_564f27, 5 May 2026; new `tenant_notification_settings` table + officer-RLS + updated_at trigger + seed for existing tenants; new `pages/admin/NotificationsAdmin.tsx` with reminder timing select + 3 toggles + Button + haptics; new `/admin/notifications` route + nav tab) |
| **CMH11-no-privacy-visibility-toggles-in-admin-config** | CMH-11, D2 | P2 | none | No "show member list publicly?", "show event RSVPs publicly?", "allow non-members to see events?" | Add `tenants.privacy_settings jsonb` (typed); admin UI; consume in Members/Events queries | M | **closed: e47b6a7** (F5, fork_mos0swkk_564f27, 5 May 2026; new `tenants.privacy_settings JSONB NOT NULL DEFAULT {show_event_attendees_publicly:false, show_member_list_publicly:false, allow_non_members_to_see_events:true}`; new `pages/admin/PrivacyAdmin.tsx` with 3 toggles; new `/admin/privacy` route + nav tab. Consumer enforcement deferred to follow-up; column populated and editable from F5) |
...
| **G2-no-optimistic-updates-on-rsvp** | G2 | P3 | `EventDetail.tsx:40-47` waits for mutation roundtrip | UX feels laggy on flaky 4G | Add optimistic update on `useRsvp`; rollback on failure with toast | S | **closed: 14332a2** (F3-redo, fork_mos0riap_f5910d, 5 May 2026; useRsvp/useUnRsvp now do onMutate cache flip + onError rollback + onSettled invalidate; EventDetail handler adds haptic + toast acknowledgement on success/failure) |
