...
beforeAll(() => {
  ...
  writeMd(path.join(clientsDir, '[redacted].md'),
    '[redacted], [redacted], fire-safety',
    '[redacted] client conventions.')
...
describe('doctrineSurface.surfaceDoctrineForPrompt', () => {
  ...
  test('caps surfaces and reports suppressed count', withCorpus((helper) => {
    const text = 'cowork-dispatch fork-default symbolic-logging [redacted] bitbucket corazon'
...
[BRIEF-CHECK WARN] anti-pattern: [redacted]
...
This is a real prompt about vacation planning.`
  ...
  test('frequency-weighted ordering: more keyword matches sort above fewer', withCorpus((helper) => {
    const text = 'bitbucket atlassian-api x-bitbucket-api-token-auth [redacted]'
    const out = helper.surfaceDoctrineForPrompt(text)
    expect(out).not.toBeNull()
    const idxBitbucket = out.indexOf('bitbucket.md')
    const idx[redacted] = out.indexOf('[redacted].md')
    expect(idxBitbucket).toBeGreaterThan(-1)
    expect(idx[redacted]).toBeGreaterThan(-1)
    expect(idxBitbucket).toBeLessThan(idx[redacted])
  }))
  ...
  test('OS_DOCTRINE_SURFACE_ENABLED=false short-circuits to null', withCorpus((helper) => {
    ...
    try {
      const out = helper.surfaceDoctrineForPrompt('cowork-dispatch [redacted] bitbucket')
      expect(out).toBeNull()
    } finally {
...
describe('doctrineSurface.matchedFiles', () => {
  test('returns structured array with file/base/dir/matchedKeywords', withCorpus((helper) => {
    const matches = helper.matchedFiles('cowork-dispatch and [redacted] work today')
