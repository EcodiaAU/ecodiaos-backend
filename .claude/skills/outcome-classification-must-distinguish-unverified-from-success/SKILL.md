---
name: outcome-classification-must-distinguish-unverified-from-success
description: >
  Use when the turn involves (no explicit triggers — surfaces via semantic match on title). Pattern: Outcome classification must distinguish `unverified` from `success` - silence is not a positive signal.
---
