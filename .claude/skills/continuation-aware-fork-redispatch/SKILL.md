---
name: continuation-aware-fork-redispatch
description: >
  Use when the turn involves continuation-check, redispatch, fork-resume, fork-orphan, in-memory-state-loss, api-crash, partial-deliverables, work-resume, fork-lost, sibling-fork-recovery, redispatch-no-duplicate, check-existing-deliverables, file-mtime-check, kv_store-deliverable-check, status_board-deliverable-check, neo4j-deliverable-check, git-commit-deliverable-check, post-crash-redispatch, conductor-redispatch. Pattern: Continuation-aware fork redispatch - check existing deliverables before duplicating.
---
