---
name: websearch-via-corazon-residential-ip-when-vps-bot-blocked
description: >
  Use when the turn involves websearch-fallback, websearch-paywall, long-context-beta, duckduckgo-curl, ddg-curl, google-search-blocked, bot-detected, captcha-blocked, residential-ip, corazon-shell, shell.shell-curl, websearch-via-corazon, websearch-broken, accept-not-fix. Pattern: When the SDK WebSearch tool is unavailable, route the search via Corazon's residential IP - do NOT accept "WebSearch is broken".
---
