---
name: neo4j-question-node-held-uncertainty
description: >
  Use when the turn involves (no explicit triggers — surfaces via semantic match on title). Pattern: Held uncertainty gets a :Question node, not an Episode footnote.
---
...
- Questions that are really TODOs ("should I email [redacted]?" is a status_board row, not a Question)
