---
name: prefer-hooks-over-written-discipline
description: >
  Use when the turn involves PreToolUse-hook, settings.json, hook-enforcement, written-discipline-fail, pre-commit-hook, pre-push-hook, remember-to-rule, every-time-rule, hook-vs-doctrine. Pattern: Prefer hooks over written discipline for recurring checks.
---
...
- `~/ecodiaos/scripts/clients/[redacted]/preflight.sh` + `.git/hooks/pre-push` on [redacted] worktree: 10+ gates including tsconfig, build, lint, emdash, string-literal-enum, dead-DTO, secrets, oversized-diff.
...
Apr 22 2026 evening Tate asked for a hooks status check. Audit found: my settings.json only had the emdash hook, [redacted] had a comprehensive pre-push preflight, Coexist had nothing, ecodiaos repo had nothing. Installed coexist pre-push preflight parallel to [redacted]'s, and added a PreToolUse hook on `mcp__factory__start_cc_session` that surfaces candidate pattern files based on the codebase being dispatched to. The Factory-dispatch hook is the direct implementation of the "surfacing is the point" doctrine Tate named the day before.
