---
name: status-board-no-batch-case-when-update
description: >
  Use when the turn involves status_board-batch-update, CASE-WHEN-update, status_board-multi-row-update, status_board-splatter, status-corruption, directive-sweep, status_board-cross-row-leak, sql-update-many-rows, status_board-sweep-protocol. Pattern: Never apply a directive sweep to status_board as a single multi-row CASE-WHEN UPDATE - one statement per row.
---
