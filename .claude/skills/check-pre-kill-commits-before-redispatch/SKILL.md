...
- In the worst case, the redispatch goes off-task entirely and produces a different artefact (real risk: 2026-04-28 Wave B redispatch produced an [redacted] bottleneck markdown brief instead of the listener modules).
...
- **0b873515 (ecodiaos-backend Wave B listeners)**: branch had ZERO listener commits pre-kill. Redispatch went off-task and produced an unrelated [redacted] brief. Rejected as phantom. Re-dispatched again as `841a5e18` with a scoped/explicit prompt.
