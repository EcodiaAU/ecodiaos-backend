---
name: system-injection-blocks-must-not-render-in-director-chat
description: >
  Use when the turn involves doctrine-surface-block, recent-doctrine-injection, relevant-memory-injection, system-reminder-leak, frontend-renders-system-blocks, chat-pollution-system-injection, hide-system-blocks-from-tate, now-block-leak, forks-rollup-leak, restart-recovery-leak, recent-exchanges-leak, scheduled-prefix-leak, conductor-only-scaffolding, sdk-prompt-vs-chat-render. Pattern: System injection blocks are conductor-only scaffolding. Never render them in the director chat..
---
