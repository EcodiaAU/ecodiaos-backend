---
name: solo-fork-pushes-to-main-no-pr-ceremony
description: >
  Use when the turn involves pr-ceremony, branch-and-pr, feature-branch, solo-fork-workflow, push-to-main, no-pr, direct-push, workflow-overkill, github-pr-flow, pr-overhead, fork-deploy-workflow, branch-pr-merge-pull-restart, simple-deploy, push-pull-restart, single-author-workflow, fork-output-pipeline, ship-direct-to-main. Pattern: Solo forks push directly to main - no PR ceremony.
---
...
- **Client repos** (ordit, fireauditors1, coexist when client owns the repo) — external reviewers (Eugene, Craige) MUST see PRs.
...
- Do NOT apply this to client repos (ordit, fireauditors1, coexist when client-owned) — those still need branch + PR for external review.
