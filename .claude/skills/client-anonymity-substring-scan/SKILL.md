---
name: client-anonymity-substring-scan
description: >
  Use when the turn involves client-anonymity, public-writing, newsletter, quorum-of-one, anonymisation, pre-publish, substring-leak, joke-reference, obfuscated-reference, case-study, pitch-deck, linkedin, blog-post, essay. Pattern: Client-anonymity must scan obfuscated and joke references, not just formal mentions.
---
...
A joke memory-key like `ceo.drafts.[redacted]-eugene-revert-two-thousand-something` dropped mid-paragraph names the client (`[redacted]`) and the reviewer (`eugene`) just as clearly as a formal sentence would. A competitor or curious stakeholder skim-reading the newsletter will pattern-match the substring. The joke does not provide cover.
...
- Before publishing any public artefact (Quorum of One, LinkedIn post, essay, blog post, case study, pitch deck, any email that may be forwarded beyond the direct recipient), run a literal substring grep against the final draft for:
  ...
  - every primary contact name from those client files (e.g. "[redacted]", "eugene", "kurt", "angelica", "jess", "ben", "matt")
  - every internal nickname or project codename currently in use (e.g. "[redacted]", "co-exist", "cetin", "resonaverde", "roam")
...
Apr 24 2026, 16:29 AEST meta-loop. Assessment of `/home/tate/ecodiaos/public/docs/quorum-of-one-003.html` (draft for Sun Apr 27 publish) caught the literal string `ceo.drafts.[redacted]-eugene-revert-two-thousand-something` in the body paragraph about drafting the client-response revert. The string had sat in the draft since Apr 23; prior sessions read the draft and did not flag it because the review passes were about voice and argument, not anonymity substring scans. The catch only happened because this meta-loop ran an explicit anonymity doctrine pass. Neo4j Pattern node id 2227; Episode id 2226.
