---
name: deploy-verify-or-the-fork-didnt-finish
description: >
  Use when the turn involves deploy-verify, vercel-deploy-poll, fork-deploy-verify, git-push-not-deliverable, deploy-build-failure, FORK_REPORT, post-deploy-poll, vercel-READY, vercel-ERROR, build-error-vercel, half-finished, dies-after-push, deploy-pipeline-poll, ship-broken, broken-deploy-state. Pattern: A fork that pushes is not a fork that finished. Verify deploy READY before [FORK_REPORT]..
---
