---
name: project-naming-mirrors-repo-name
description: >
  Use when the turn involves vercel-project-name, vercel-cli-link, vercel-link-cli, vercel-project-create, project-naming, project-naming-mirrors-repo, fe-project-name, be-project-name, vercel-naming-convention, projectName, vercel-scaffold-naming, PROJECT_NAMING. Pattern: Vercel projects (and any cloud project name) mirror the GitHub repo name. Never `fe`, never `be`, never the directory's relative name..
---
...
**The Vercel project name MUST equal the GitHub repo name.** GitHub repo `chambers-frontend` -> Vercel project `chambers-frontend`. GitHub repo `[redacted]-backend` -> Vercel project `[redacted]-backend`. Never the relative directory name (`fe`, `be`, `frontend`, `app`). Never the working-copy convention. The cross-system identifier is the repo name.
