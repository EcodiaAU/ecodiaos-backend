---
name: corazon-is-a-peer-not-a-browser-via-http
description: >
  Use when the turn involves corazon, laptop-agent, tailscale, 100.114.219.69, browser.*, puppeteer-wrapper, cdp-attach, chrome-profile, ms-teams-app, desktop-app-screenshot, filesystem.*, input.*, native-desktop, electron-app, slack-desktop, discord-desktop, vs-code-app, cursor-app, xcode-app, ssh-corazon, ssh-windows, openssh-server, peer-not-wrapper, native-input, drive-via-keyboard, cookie-file-direct-read, sqlite-cookies, profile-directory, chrome_profile_dir, multi-account-chrome, profile-1, default-profile, ms-teams-desktop. Pattern: Corazon is a peer to the VPS, not a "browser via HTTP" wrapper.
---
