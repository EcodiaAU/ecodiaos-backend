---
name: enumerate-all-trigger-paths-when-fixing-data-flow-bugs
description: >
  Use when the turn involves data-flow-bug, sync-bug, task-not-clearing, multiple-paths, trigger-coverage, enumerate-paths, source-table, partial-fix, missed-path, forms-canonical, app-canonical, sheet-sync, excel-sync, surveys, leader-tasks, event_impact, multiple-producers, fix-coverage. Pattern: Enumerate ALL trigger paths when fixing data-flow bugs, not just the obvious one.
---
