---
name: grace-timer-must-not-kill-chat-session
description: >
  Use when the turn involves (no explicit triggers — surfaces via semantic match on title). Pattern: Grace-timer process.exit MUST be opt-in, not default.
---
