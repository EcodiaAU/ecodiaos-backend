---
name: conductor-coordinates-capacity-is-a-floor
description: >
  Use when the turn involves conductor-coordinates, executor-mode, fork-capacity-floor, fork-slots, capacity-floor, work-doer, route-not-execute, parallel-builder, prudence-disguising-idleness, COORDINATING, fill-the-slot, conductor-underutilising. Pattern: Conductor coordinates - capacity is a floor, not a ceiling.
---
...
The remediation in the same turn: spawned three forks (listeners audit+build, status board drift cleanup, [redacted] unbilled-hours brief) bringing total to 4/5. Updated parallel-builder cron prompt with lesson #4 ("unused fork capacity is the failure mode this cron exists to prevent"). Wrote this pattern.
