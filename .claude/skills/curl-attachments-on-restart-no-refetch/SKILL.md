---
name: curl-attachments-on-restart-no-refetch
description: >
  Use when the turn involves (no explicit triggers — surfaces via semantic match on title). Pattern: Curl attachments to disk on restart, do not re-invoke WebFetch.
---
