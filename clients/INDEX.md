...
This directory holds one `.md` file per client (Co-Exist, [redacted], ...) and per system Tate operates (Corazon, SY094 MacInCloud) plus shared release-flow doctrine that applies across multiple Ecodia/client apps. Files have a YAML-ish `triggers:` frontmatter line declaring relevance scope, mirroring the `~/ecodiaos/patterns/` and `~/ecodiaos/docs/secrets/` convention.
...
- Any client-facing email, PR comment, Bitbucket reply, or Slack DM (Co-Exist, [redacted], Resonaverde, Landcare, future clients)
...
| ~~[redacted].md~~ | _ARCHIVED 2026-05-17_ | [redacted] / [redacted] no longer a client. Historical files at `archived/[redacted].md` + `archived/[redacted]/`. Pattern `[redacted]-prepush-pipeline.md` moved to `patterns/_archived/`. Skill moved to `.claude/skills/_archived/`. Do not surface as active. |
