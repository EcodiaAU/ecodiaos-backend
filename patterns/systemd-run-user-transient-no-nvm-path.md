...
# systemd-run --user transient units don't source nvm/login shell rc â€” invoke nvm-installed binaries (pm2, node, npx) by absolute path or via `bash -lc`
...
- Do **not** use `/bin/bash -c '<cmd>'` (no `-l`) for nvm binaries â€” that runs a non-login non-interactive shell with NO rc files sourced.
...
- Do **not** drop into a fresh shell to test whether nvm loads â€” bash inside an interactive ssh session DOES source rc files; systemd transient units don't. Test inside the actual scheduling form, not in a regular shell.
...
- 21:30 AEST: meta-loop continuation observed `pm2 jlist` showed ecodia-api still on the old pm_uptime (12h+). Probed `systemctl --user status` for the v1 unit â€” saw exit 127.
...
9 May 2026 meta-loop turn (21:05 AEST orient â†’ 21:14 v1 fail â†’ 21:30 v2 ship). v1 unit-cmd `bash -c 'pm2 restart ecodia-api'` exited 127 because the v1 unit's `bash -c` shell did not have nvm's pm2 on PATH. Diagnosed via `systemctl --user status <v1-unit>`, fixed via absolute path + `bash -lc`. Pattern authored same arc per `~/ecodiaos/patterns/codify-at-the-moment-a-rule-is-stated-not-after.md`.

Stamped: meta-loop turn 9 May 2026 21:30 AEST.

## Cross-references

- `~/ecodiaos/patterns/never-schedule-host-process-restart-via-os-scheduled-tasks.md` â€” the rule that mandates out-of-band scheduling. systemd-run is the recommended substrate; this pattern is its operational gotcha.
- `~/ecodiaos/patterns/dispatcher-fix-on-disk-does-not-equal-fix-in-running-process.md` â€” the rule that motivates the restart in the first place. Both patterns authored same meta-loop arc 9 May 2026.
- `~/ecodiaos/patterns/_archived/pre-stage-fork-briefs-before-session-killing-ops.md` â€” pair this with handoff pre-staging since the conductor session itself dies on the eventual successful restart.
- `~/ecodiaos/patterns/_archived/no-pm2-restart-during-active-factory-queue.md` â€” re-check Factory queue + forks rollup BEFORE the v2 schedule fires (still applies; v1 fail is unrelated to queue).
- `~/ecodiaos/patterns/verify-deployed-state-against-narrated-state.md` â€” the meta-rule. v1 was "scheduled" but not "ran"; only the post-fire `systemctl status` exit code reveals which.
