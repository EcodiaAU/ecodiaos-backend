---
triggers: cred-rotation, credential-rotation, rotation-propagation, kv_store-creds, vercel-env, supabase-secrets, edge-function-secrets, smtp-rotation, resend-smtp, credential-update, all-consumers, rotate-credential, rotate-key, key-rotation, secret-rotation, vendor-rotation, oauth-rotation, api-key-rotation, password-rotation, deploy-env-vars, supabase-edge-secrets, secrets-list, multi-surface-rotation, post-rotation-audit, downstream-consumers
---
...
<!-- Trigger narrowing 2026-05-21 (self-evolution routine).
     OLD: rotation, propagation, kv_store, vercel-env, supabase-secrets, edge-function, smtp, resend, credential-update, all-consumers, cred-rotation, rotate-credential, rotate-key, key-rotation, secret-rotation, vendor-rotation, smtp-rotation, oauth-rotation, api-key-rotation, password-rotation, deploy-env-vars, supabase-edge-secrets, secrets-list, multi-surface-rotation, post-rotation-audit, downstream-consumers
     NEW: cred-rotation, credential-rotation, rotation-propagation, kv_store-creds, vercel-env, supabase-secrets, edge-function-secrets, smtp-rotation, resend-smtp, credential-update, all-consumers, rotate-credential, rotate-key, key-rotation, secret-rotation, vendor-rotation, oauth-rotation, api-key-rotation, password-rotation, deploy-env-vars, supabase-edge-secrets, secrets-list, multi-surface-rotation, post-rotation-audit, downstream-consumers
     Why: removed 4 bare-noun triggers (rotation, propagation, smtp, resend) per triggers-must-be-narrow-not-broad.md. `rotation` -> `credential-rotation` (compound with the subject). `propagation` -> `rotation-propagation` (compound naming the failure mode). `smtp` -> `smtp-rotation` (smtp alone fires on every SMTP/email brief). `resend` -> `resend-smtp` (Resend is the SaaS vendor; the bare word `resend` is also a verb fragment matching too liberally - `resend-smtp` scopes to the credential context). `kv_store` -> `kv_store-creds` (matches the actual subject - kv_store-stored credentials, not every kv_store row). `edge-function` -> `edge-function-secrets` (same narrowing). -->
