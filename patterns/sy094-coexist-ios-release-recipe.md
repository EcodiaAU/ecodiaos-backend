...
For fully-autonomous reruns where no human review is possible AND skipping the visual smoke-test is acceptable. **All commands run from inside Terminal.app on SY094 (entered via the RDP session per Phase A + Phase B).** Drive each line via `input.type` + Enter through the on-Mac eos-laptop-agent (which itself was spawned from inside the RDP terminal so it inherits GUI Aqua context); or, if the operator is sitting at the RDP keyboard, type directly. SSH-from-VPS is forbidden per `~/ecodiaos/patterns/macincloud-substrate-selection-ssh-vs-rdp.md`.
