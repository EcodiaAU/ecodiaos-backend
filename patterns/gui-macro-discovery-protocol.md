...
| ssh sy094, sshpass sy094, "ssh into macincloud", ssh-vs-rdp, headless-vs-gui-on-mac, remote-build-port | [`macincloud-substrate-selection-ssh-vs-rdp.md`](macincloud-substrate-selection-ssh-vs-rdp.md) (substrate-selection rule - SSH for headless work, RDP for GUI-bound work; supersedes the 5 May absolute SSH ban) | n/a |
...
1. **Reaching for `sshpass -p ... ssh user276189@SY094.macincloud.com` when the user says "open macincloud".** "Open macincloud" implies the GUI session, which is RDP-only per `macincloud-substrate-selection-ssh-vs-rdp.md`. The RDP shortcut on Corazon's desktop is the canonical path and is already verified at 23.6s. SSH is now authorised for headless work (since 7 May 2026) but not for "open macincloud" GUI flows. The grep would have surfaced this immediately.
...
- [`macincloud-substrate-selection-ssh-vs-rdp.md`](macincloud-substrate-selection-ssh-vs-rdp.md) - SY094 substrate-selection rule (7 May 2026, supersedes 5 May absolute SSH ban): SSH for headless work, RDP for GUI-bound work
