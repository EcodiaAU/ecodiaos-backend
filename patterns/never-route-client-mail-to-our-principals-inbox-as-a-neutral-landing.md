...
- `~/ecodiaos/patterns/_archived/decide-do-not-ask.md` - override case. The default is "decide, do not ask", but when the question is "what is the client's destination?" the answer is unavailable to the conductor by construction. Decide-do-not-ask does not license inventing answers that are not yours to invent.
