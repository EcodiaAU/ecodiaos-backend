...
4. **Manager calls `mcp__forks__wait_for_sub_forks` to block its turn.** ONE call per consolidation cycle, with `sub_fork_ids` = every worker spawned and `max_wait_sec` = expected upper bound for the slowest worker (default 1800s, cap 3600s). The tool blocks server-side, polling `os_forks` every 5s, and returns when every worker reaches a terminal status (or on timeout). The blocking tool call keeps the manager's SDK turn alive STRUCTURALLY — there's no idle-turn termination because a tool is in flight. When the tool returns, the manager has aggregated `{fork_id, status, result_head, next_step}` for every worker in one structured payload. Sequence dependent steps by spawning a fresh batch + calling `wait_for_sub_forks` again on the new ids; do not spawn step-2 until the step-1 wait returns. On timeout the payload includes `still_pending: [...]` and the manager decides: call again, abort stragglers, or proceed with what shipped. Retry phantom-bails (status=`done` but `result_head` carries the FALLBACK_MARKER prefix) by probing deliverables (`git log --grep=<fork_id>`, `ls -la <expected_artefact_path>`) before re-dispatching.
5. **Do NOT hand-roll a polling loop with `list_forks` / `db_query os_forks`.** That pattern is what `wait_for_sub_forks` replaces. Pre-tool, ~37% of manager forks phantom-bailed because the model emitted no further tool calls between polls, the SDK turn ended naturally with `terminal_reason='completed'`, the iterator closed, and `[FORK_REPORT]` was never written. `list_forks` and `db_query os_forks` remain useful for one-off spot-checks (e.g. mid-decomposition, before deciding whether to spawn the next batch) but never as a substitute for the blocking wait. Origin: 19/51 manager forks (37%) phantom-bailed in the 7d ending 10 May 2026, ~374k tokens wasted in re-dispatch cycles. Decision doc: `~/ecodiaos/docs/decisions/manager-fork-bail-architecture-decision-2026-05-10.md`.
6. **Manager verifies.** Sub-fork self-reports lie. Read each sub-fork's durable artefact (the file path the sub-fork named in its `[SUB_FORK_REPORT]`, typically under `~/ecodiaos/drafts/<artefact>.md` or a commit SHA), then confirm deployed state, committed code, or DB rows match the claim before consolidating.
7. **Manager consolidates.** ONE `[FORK_REPORT]` to the conductor: what shipped, what didn't, what the conductor should do next. Never multiple reports — that's noise the manager-fork pattern exists to prevent.
...
- `~/ecodiaos/docs/decisions/manager-fork-bail-architecture-decision-2026-05-10.md` — the architecture decision that authorised `mcp__forks__wait_for_sub_forks` (Section 4 carries the implementation sketch + variant comparison).
