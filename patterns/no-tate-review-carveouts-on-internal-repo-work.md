...
- Do NOT open a PR as the default ship mechanic on internal repos. PRs are appropriate ONLY when (a) the work is on a CLIENT repo with PR review as part of the contract (Ordit, etc.), or (b) the work is exploratory and the diff itself is the conversation.
