...
Four load-bearing cases, each a real failure:
...
- **Bug "fixed" without reproducing on the old build.** Running only the NEW code and seeing correct behaviour returns the same green when the bug never existed on that path, was environment-specific, or was resolved by something else (a cache clear, a sibling's commit, a dependency bump). The probe must first REPRODUCE the failure on the old build or commit, then show the new build passing the identical steps. Without the reproduction the honest claim is "behaviour is correct now", and the real bug may still be live on the path the user hit.
