...
6. **Build VALID + export compliance.** Poll `GET /v1/builds` until `processingState=VALID`. Then `PATCH /v1/builds/{id} {usesNonExemptEncryption:false}` - without this the build shows "Missing Compliance" and is NOT installable. (Or set `ITSAppUsesNonExemptEncryption=false` in Info.plist to avoid it entirely.)
...
8. **Internal TestFlight (instant, no review).** `POST /v1/betaGroups {isInternalGroup:true}` -> assign build -> `POST /v1/betaTesters` for each team ASC user (account holder + devs). Internal testers install immediately, no beta review.
