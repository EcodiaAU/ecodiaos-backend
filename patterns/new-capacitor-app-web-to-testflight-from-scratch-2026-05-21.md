...
8. **Internal TestFlight (instant, no review).** Three sub-steps - all three mandatory, the third one is the easy thing to forget and the only reason a tester reports "I only see the old build":
   1. **Create the internal group:** `POST /v1/betaGroups {name:"Ecodia Team", isInternalGroup:true}` (relate `apps` to the new ASC app id). The name "Ecodia Team" is the convention - keep it consistent so `ship-ios.py`'s `filter[isInternalGroup]=true` lookup is unambiguous.
