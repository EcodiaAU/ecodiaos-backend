...
8. **Internal TestFlight (instant, no review).** Three sub-steps - all three mandatory, the third one is the easy thing to forget and the only reason a tester reports "I only see the old build":
   1. **Create the internal group - DASHBOARD ONLY, not the API.** `POST /v1/betaGroups {isInternalGroup:true}` silently creates an EXTERNAL group (`isInternalGroup` comes back `false`) and the `PATCH` to fix it returns `409 NOT_ALLOWED`. This is a one-time per-app dashboard step: TestFlight tab -> Internal Testing -> "+" -> name "Ecodia" -> Enable automatic distribution -> add the team users (~30s). Keep the name consistent so `ship-ios.py`'s `filter[isInternalGroup]=true` lookup is unambiguous. Do NOT send external invite emails to the ASC team users as a workaround - it cross-associates their Apple ID and blocks install. Full failure analysis: `asc-internal-beta-group-must-be-created-via-dashboard-not-api.md`.
   2. **Add the three canonical Ecodia internal testers** via `POST /v1/betaTesters` (relate `apps` + `betaGroups`):
      ...
      All three are already registered as ASC team users on the Apple Developer team (`86PUY7393S`); no per-app user creation needed. The gmail address is the dotted form - Gmail ignores dots so either spelling reaches the inbox, but Apple uses email as identifier and the dotted spelling is what's on file across all Ecodia apps, so stay consistent.
   3. **Attach the build to the internal group** via `POST /v1/betaGroups/{group_id}/relationships/builds` with `{"data":[{"type":"builds","id":"{build_id}"}]}`. **This is the step that flips `internalBuildState` from `READY_FOR_BETA_TESTING` to `IN_BETA_TESTING` and notifies internal testers.** Without it the build sits VALID but invisible to testers, who keep seeing the previous build. The 21 May 2026 build-4 incident traced to this exact omission - the driver uploaded + processed VALID but never attached, so `tate.donohoe@gmail.com` only saw build 3. `ship-ios.py` step `[8b]` now does this automatically on every upload (idempotent: 422 "already exists" treated as success). For external groups + public links the build-to-group attach is the same shape but uses the external group's id (see step 9).
