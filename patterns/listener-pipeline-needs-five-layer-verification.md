...
1. **Producer** â€” code that actually inserts rows / updates state in the watched table (e.g. gmail polling code â†’ `INSERT INTO email_events`).
2. **Trigger** â€” `pg_trigger` row that fires `pg_notify('eos_listener_events', ...)` on the producer's write.
3. **Bridge** â€” `dbBridge.js` LISTEN connection routing pg_notify into wsManager broadcast (`db:event` channel).
4. **Listener** â€” the module in `src/services/listeners/` that exports `{name, subscribesTo, relevanceFilter, handle, ownsWriteSurface}` and is loaded by the registry.
5. **Side-effect** â€” the handler's actual output (kv_store key written, message_queue row enqueued, status_board updated, OS session woken via POST /api/os-session/message, SMS sent, file written, etc.).
...
- Layer 2 (trigger): `trg_email_events_insert_notify` exists in `pg_trigger` âœ“
- Layer 3 (bridge): `dbBridge.js` is LISTENing on `eos_listener_events` âœ“
- Layer 4 (listener): `emailArrival.js` loads cleanly, exports valid shape, registry registered it âœ“
- Layer 5 (side-effect): handler is wired to POST `/api/os-session/message` with the wake-message âœ“
...
So the listener has been technically "shipped" since 2026-04-28 04:46 AEST, but has fired exactly 0 times because nothing produces the events it watches for. That's 18+ hours of dark time hidden under "wired correctly." A single-layer audit (e.g. "is the listener loaded? yes. âœ“ ship it.") would never catch this.
...
2. **Trigger second.** Confirm via `SELECT * FROM pg_trigger WHERE tgname LIKE '<expected_name>'`. Migration file existing on disk is not enough â€” the migration must actually be applied. Check `_migrations` table for the filename.
...
4. **Listener fourth.** Confirm registry loaded it: `node -e "const r=require('./src/services/listeners/registry'); r.loadListeners().forEach(l=>console.log(l.name))"` â€” your listener should appear in the output. Also confirm the running api process loaded it (check side-effect artefacts, NOT log files â€” see `verify-empirically-not-by-log-tail.md`).
...
1. Query the watched table for recent activity. If empty over the expected timeframe, the producer is likely missing or broken â€” go check layer 1 first.
...
- Use `pm2 logs --lines N` as listener-firing evidence. Log capture can break silently. Use empirical artefacts (DB rows, kv_store keys, message_queue entries) â€” see `verify-empirically-not-by-log-tail.md`.
...
[ ] LAYER 5 SIDE-EFFECT:
    ...
    smoke test: insert synthetic row matching trigger â†’ expected artefact appears within N seconds â†’ cleanup synthetic
```
...
Apr 28 2026 22:55 AEST. Listener audit during Tate's "fully utilise listeners" directive. emailArrival found dark â€” layers 2-5 correct, layer 1 absent. Decision 3611 followed: dispatch fork_moin0lrp to wire the producer (gmail polling â†’ email_events insert) so the listener becomes load-bearing. The audit also surfaced that other 4 listeners (forkComplete, statusBoardDrift, invoicePaymentState, factorySessionComplete) had producers in place AND fired today, validating the 5-layer model in the negative direction (when all 5 layers are present, the listener works).

## Cross-references

- Pattern: `verify-empirically-not-by-log-tail.md` (use empirical artefact checks, not log tails, for layer-5 evidence).
- Pattern: `_archived/pre-stage-fork-briefs-before-session-killing-ops.md` (when shipping a listener that requires PM2 restart, sequence the restart safely).
